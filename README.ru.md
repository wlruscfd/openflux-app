...
На главном экране есть два способа пустить трафик через активный профиль:

- **VPN (по умолчанию)** — `OpenFluxVpnService` захватывает всё устройство через Android
  `VpnService`, как и раньше. Один раз запрашивает системное согласие на VPN.
- **Локальный SOCKS5-прокси** — `OpenFluxSocks5Service` поднимает обычный SOCKS5-прокси
  (только TCP CONNECT, без авторизации) на `127.0.0.1` (порт настраивается в Настройках,
  по умолчанию 1080) через тот же транспорт, вообще без VpnService/TUN/запроса согласия —
  через него идёт трафик только того приложения, которое вы явно на него настроите, всё
  остальное на устройстве идёт своим обычным путём. Аналог `--socks5`-режима десктопного
  клиента — см. пакет `socks5/` в [openflux-server](https://github.com/wlruscfd/openflux-server).
  Имя хоста в SOCKS5 CONNECT резолвится обычным DNS-резолвером устройства до того, как
  соединение попадёт в туннель — туннелируется только результирующий IP-трафик, а не сам
  DNS-запрос. Взаимоисключим с VPN-режимом: попытка включить один при работающем другом
  завершится ошибкой.
